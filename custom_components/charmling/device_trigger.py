"""Device triggers: the moments, in the automation editor's device picker.

"When Abi's MacBook Pro… the dog woofed" — chosen from a list, no event
data to type. Each is the bus event `charmling_event` filtered to this
device and one moment type.
"""
...
from .const import DOMAIN, EVENT_NAME, EVENT_TYPES
...
async def async_get_triggers(hass: HomeAssistant, device_id: str) -> list[dict[str, Any]]:
    """Every moment, for a Charmling device."""
    device = dr.async_get(hass).async_get(device_id)
    if device is None or not any(ident[0] == DOMAIN for ident in device.identifiers):
        return []
    return [
